# Create/update desktop shortcut for StarSim
# RULE: Single canonical location = dist-electron\StarSim.exe (exe, shortcut, and web all sync to same build)
...
# SINGLE SOURCE OF TRUTH: dist-electron\StarSim.exe (build-all copies win-unpacked here)
...
if ($isPlaceholder -and (Test-Path "$projectRoot\dist-electron\StarSim-Launcher.ps1")) {
    # Placeholder "exe" is batch script - not runnable. Use PowerShell launcher instead
    $targetPath = (Get-Command powershell).Source
    $workDir = "$projectRoot\dist-electron"
    $shortcutArgs = "-ExecutionPolicy Bypass -WindowStyle Hidden -File `"$projectRoot\dist-electron\StarSim-Launcher.ps1`""
...
if (-not $targetPath) {
    Write-Host "No StarSim launcher found - skipping desktop shortcut" -ForegroundColor Yellow
    exit 0
}
...
try {
    ...
    $Shortcut.Description = "StarSim - Interactive Cosmic Simulator"
    $Shortcut.Save()
